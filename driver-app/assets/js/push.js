...
const DriverPush = (() => {
  ...
  function showToast(payload) {
    ...
    if (Notification.permission === "granted") {
      new Notification(title || "BusTrack Driver", {
        body: body || "",
        icon: "/driver-app/assets/icons/icon-192.png",
        tag: data.type || "bustrack-driver",
      });
    }
    ...
    toast.style.cssText = `
      ...
      background:#1a2540;border:1px solid #0ea5e9;border-radius:12px;
      padding:14px 20px;display:flex;align-items:center;gap:12px;
      z-index:9999;box-shadow:0 8px 24px rgba(0,0,0,0.4);
      max-width:380px;width:90%;animation:slideDown 0.3s ease;cursor:pointer;
    `;
    toast.innerHTML = `
